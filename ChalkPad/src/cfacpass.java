 ...
 * @author architgarg
 */
public class cfacpass extends JFrame implements ActionListener,ItemListener{
    JLabel lop,lnp;
    JTextField top,tnp;
    JButton bsbm,bb,bh;
    String sid,snm;
    cfacpass(String s1,String s2)
    {    snm=s1;
         sid=s2;
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if(e.getSource()==bb)
        {
            this.dispose();
            dashfac ds=new dashfac(snm,sid);
        }
      //Home Button
              if(e.getSource()==bh)
        {
        ...
        PreparedStatement ps=con.prepareStatement("select * from fac where pass=? and id=?");
        ps.setString(1,top.getText());
        ps.setString(2,sid);
        ResultSet rs=ps.executeQuery();
        if(rs.next())
        {
        ps=con.prepareStatement("update fac set pass='"+tnp.getText()+"' where id='"+sid+"' and pass='"+top.getText()+"'");;
        ps.executeUpdate();
